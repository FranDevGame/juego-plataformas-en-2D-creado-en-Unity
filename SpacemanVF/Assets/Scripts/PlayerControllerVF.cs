 ...
 * en la posición inicial que hemos predefinido.*/
    ...
    /*Hemos modificado el código para el salto del personaje, de manera que hemos sustituido el GetKeyDwon por el Getbutton con el nombre
     *predefinido por la acción de Jump, que es la barra espaciadora, en este caso predefinido por el InputManager de Unity. 
     *Tenemos captura del código y la explicación en el ordenador.*/
    /* Hemos arraigado el if Jump dentro de otro if que sentencia lo siguiente:
     * Si el currentGameState de la instancia compartida del GameManager está en Estado de juego inGame, el personaje saltará.*/
    void Update()
    {
        ...
        if (Input.GetButtonDown("SuperJump"))/* Si presionamos el botón del InputManager con el nombre SuperJump, que en este caso es el botón derecho del ratón, la variable booleana SuperJump,
